...
class MnistNeqModel_add2(nn.Module):
    def __init__(self, model_config):
        super(MnistNeqModel_add2, self).__init__()
        ...
        for i in range(1, len(self.num_neurons)):
            ...
            if i == 1:
                ...
                input_quant = QuantBrevitasActivation(
                    QuantHardTanh(
                        bit_width=self.model_config["input_bitwidth"],
                        max_val=1.0,
                        min_val=-1.0,
                        narrow_range=False,
                ...
                output_quant = QuantBrevitasActivation(
                    QuantIdentity(
                        bit_width=model_config["hidden_bitwidth"]+1,
                        quant_type=QuantType.INT,
                    ),
                )
                ...
                output_quant = QuantBrevitasActivation(
                    QuantIdentity(
                        bit_width=model_config["output_bitwidth"]+1,
                        quant_type=QuantType.INT,
                    ),
                )
                output_quant_adder = QuantBrevitasActivation(

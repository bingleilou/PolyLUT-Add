...
from polylut.quant import QuantBrevitasActivation
from polylut.nn import (
    SparseLinearNeq,
    SparseLinearNeq_add2,
    Adder2,
    ScalarBiasScale,
...
class UnswNb15NeqModel_add2(nn.Module):
    def __init__(self, model_config):
        super(UnswNb15NeqModel_add2, self).__init__()
        ...
        for i in range(1, len(self.num_neurons)):
            ...
            if i == 1:
                ...
                output_quant = QuantBrevitasActivation(
                    QuantIdentity(
                        bit_width=model_config["hidden_bitwidth"]+plus,
                        quant_type=QuantType.INT,
                    ),
                )
                output_quant_adder = QuantBrevitasActivation(
                    QuantReLU(
                ...
                imask = FeatureMask(
                    in_features,
                    out_features,
                    fan_in=model_config["input_fanin"]*2,
                    degree=model_config["degree"],
                )
                layer = SparseLinearNeq_add2(
                    in_features,
                    out_features,
                ...
                imask = FeatureMask(
                    in_features,
                    out_features,
                    fan_in=model_config["output_fanin"]*2,
                    degree=model_config["degree"],
                )
                layer = SparseLinearNeq_add2(
                    in_features,
                ...
                imask = FeatureMask(
                    in_features,
                    out_features,
                    fan_in=model_config["hidden_fanin"]*2,
                    degree=model_config["degree"],
                )
                layer = SparseLinearNeq_add2(
                    in_features,
...
class UnswNbVerilogModel(UnswNb15NeqModel):
    pass

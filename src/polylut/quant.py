...
class QuantBrevitasActivation(nn.Module):
    def __init__(
        self, brevitas_module, pre_transforms: list = [], post_transforms: list = [], last_post_transforms: list = []
    ):
        ...
        self.last_post_transforms = nn.ModuleList(last_post_transforms)
    ...
    def apply_last_post_transforms(self, x):
        for i in range(len(self.last_post_transforms)):
            x = self.last_post_transforms[i](x)
        return x
